...
app.directive("pigeonTable", function ($parse, $http) {
    ...
    direc.templateUrl = 'pigeon-table/template/outputTemplateTable.html';
...
app.controller("InsertModalInstanceCtrl", function ($scope, $http, $uibModalInstance, selectedData, tableStructure) {
    ...
    $scope.readOnly = [false,false,true];
    
    var p;
    
    for (p in selectedData) {
        $scope.tableDataKeys.push(p);
    } 
    ...
    for (i = 0; i < Object.keys(selectedData).length - 2; i++) {
        $scope.form[Object.keys(selectedData)[i]] = "";
    }
    $scope.form[Object.keys(selectedData)[2]] = "available"
    ...
    $scope.submit = function () {
        ...
        $http.post("pigeon-core/get-data-with-crud-table.php", JSON.stringify(form))
            .then(function (response) {
...
app.controller("EditModalInstanceCtrl", function ($scope, $http, $uibModalInstance, selectedData, tableStructure) {
    ...
    $scope.tableStatus = ["available", "occupied", "reserved"];
